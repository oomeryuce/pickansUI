<template>
  <div class="col-span-12 mt-2 mb-3">
    <ul class="menu py-4 shadow-md bg-base-100 rounded-box">
      <li class="menu-title">
        <span class="capitalize"> {{ $t("settings.title") }} </span>
      </li>
      <li
        :class="
          active === 'profile'
            ? 'bordered text-pickans-light'
            : 'hover-bordered'
        "
      >
        <a @click="changeTab('profile')">
          <svg
            class="w-6 h-6 mr-4 opacity-75 fill-current"
            viewBox="0 0 496 512"
          >
            <path
